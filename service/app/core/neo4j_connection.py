...
class NEO4J_Connection:
    ...
    def _initialize_connection(self, num_tries=5, try_delay=2):
        ...
        for i in range(num_tries):
            try:
                self.neo4j_db = neo4j.GraphDatabase.driver(self.server_url, auth=(self.user, self.pwd), encrypted=False)
    ...
    def query(self, query, parameters=None, db=None):
        ...
        response = None
        try: 
            session = self.neo4j_db.session(database=db) if db is not None else self.neo4j_db.session() 
    ...
    def create_index_on_node_property(self, label, attribute):
        ...
        self.query('CREATE CONSTRAINT IF NOT EXISTS ON (n:{}) ASSERT n.{} IS UNIQUE'.format(label, attribute))
    ...
    def bulk_create_nodes(self, label, on_attribute, rows, batch_size):
        ...
        while batch * batch_size < len(rows):
            res = self.query(query, parameters={'rows': rows[batch*batch_size:(batch+1)*batch_size]})
            total += res[0]['total']
            batch += 1
        print('completed loading {} {} nodes in {} seconds'.format(total, label, time()-start))
        return query

    def bulk_create_relations(self, name, source_label, source_attr, target_label, target_attr, rows, batch_size):
        # Function to handle the updating the Neo4j database in batch mode.
        total = 0
        batch = 0
        start = time()
        ...
        while batch * batch_size < len(rows):
            ...
            total += res[0]['total']
            batch += 1
        print('completed loading {} {} relation in {} seconds'.format(total, name, time()-start))
        return query
    ...
    def get_node_attribute_values_no_relation(self, label, match_attribute, source_relation, extraction_label, extraction_relation, extractor_label, extractor):
        query = ' MATCH (n:' + label + ') WHERE NOT (n)<-[:'+ source_relation +']-(:'+ extraction_label +')-[:'+ extraction_relation +']->(:'+extractor_label+' {uuid:"'+extractor['uuid']+'"}) RETURN n.'+ match_attribute +' as '+ match_attribute +' '
        results = self.run_query(query)
    ...
    def get_filtered_group_by_sentiment(self, entity_node, entity_value, constaint_node, constraint_extraction_value, source_node, source_extraction_value):
        results = self.run_query(('MATCH (o:OPINION)<-[:HAS_OPINION]-(e:EXTRACTION {title:\'sampo_extraction\'})-[ef:EXTRACTED_FROM]->(r:REVIEW)-[oc:ON_COMPANY]-(c:COMPANY {name:\''+entity_value+'\'})') + 
                ('MATCH (a:ASPECT)<-[:HAS_ASPECT]-(e:EXTRACTION {title:\'sampo_extraction\'})-[ef:EXTRACTED_FROM]->(r:REVIEW)-[oc:ON_COMPANY]-(c:COMPANY {name:\''+entity_value+'\'})') + 
                ('Return a, o'))

        return results
        get_filtered_sentiment

    def get_filtered_sentiment(self, entity_node, entity_value, source_node, source_extraction_value):
        results = self.run_query(('MATCH (o:OPINION)<-[:HAS_OPINION]-(e:EXTRACTION {title:\'sampo_extraction\'})-[ef:EXTRACTED_FROM]->(r:REVIEW)-[oc:ON_COMPANY]-(c:COMPANY {name:\''+entity_value+'\'})')+ 
                ('Return o'))
    ...
    def get_report(self):
        results = self.run_query(('MATCH (m:MODIFIER)<-[:HAS_MODIFIER]-(e:EXTRACTION {title:\'sampo_extraction\'})-[ef:EXTRACTED_FROM]->(r:REVIEW)-[oc:ON_COMPANY]-(c:COMPANY)') + 
    ...
    ### helper functions
    def create_node(self, type_, attributes, flatten=True, flattenList=True, agent_uuid=None):
        if flatten:
            attributes = json_utils.flatten_json(attributes, flattenList=flattenList)

        node = self._write_transaction(self._create_node, type_, attributes, agent_uuid)
    ...
    def set_node_attributes(self, uuid, attributes, flatten=True, flattenList=True, agent_uuid=None):
        if flatten:
            attributes = json_utils.flatten_json(attributes, flattenList=flattenList)
        node = self._write_transaction(self._set_node_attributes, uuid, attributes, agent_uuid)
    ...
    def get_nodes_by_type_attribute_value(self, type, attribute, value, single=False, skip=None, limit=None):
        attributes = {}
        attributes[attribute] = value
        nodes = self.get_nodes_by_label_attributes(attributes, label=type, single=single, skip=skip, limit=limit)
        return nodes

    def get_count_by_type_attribute_value(self, type, attribute, value, single=False, skip=None, limit=None):
        attributes = {}
        attributes[attribute] = value
        nodes = self.get_nodes_by_label_attributes(attributes, label=type, single=single, skip=skip, limit=limit)
        return len(nodes)
    ...
    def get_nodes_by_label_attributes(self, attributes, label=None, single=False, skip=None, limit=None, order_by=None, order_dir='ASC'):
        nodes = self._read_transaction(self._get_node_by_label_attributes, attributes, label, skip, limit, order_by, order_dir)
    ...
    def create_relation(self, relation, attributes, from_uuid, to_uuid, flatten=True, flattenList=True, agent_uuid=None):
        ...
        rel = self._write_transaction(self._create_relation, relation, attributes, from_uuid, to_uuid, agent_uuid)
    ...
    def delete_relation(self, relation, from_uuid, to_uuid):
        self._write_transaction(self._delete_relation, relation, from_uuid, to_uuid)
    ...
    def get_relations_between(self, from_uuid, to_uuid, relation=None):
        rels = self._read_transaction(self._get_relations_between, from_uuid, to_uuid, relation)
    ...
    def set_relation_attributes(self, relation, attributes, from_uuid, to_uuid, flatten=True, flattenList=True, agent_uuid=None):
        if flatten:
            attributes = json_utils.flatten_json(attributes, flattenList=flattenList)
        rel = self._write_transaction(self._set_relation_attributes, relation, attributes, from_uuid, to_uuid, agent_uuid)
    ...
    def get_nodes_by_relation(self, uuid, relation=None, type=None, single=False, skip=None, limit=None):
        nodes = self._read_transaction(self._get_nodes_by_relation, uuid, relation, type, skip, limit)
    ...
    def count_nodes_by_relation(self, uuid, relation=None, type=None):
        result = self._read_transaction(self._count_nodes_by_relation, uuid, relation, type)
    ...
    def get_node_types(self):
        result = self.run_query(' MATCH (n) RETURN distinct labels(n) as nodeType')
    ...
    def get_nodes_by_relation_and_type(self, source_label, source_filter, source_filter_value,
                                            dest_label, dest_filter, dest_filter_value,
                                            relation, return_type, count_type):
        result = self._read_transaction(self._get_nodes_by_relation_and_type, source_label, source_filter, source_filter_value,
                                            dest_label, dest_filter, dest_filter_value,
                                            relation, return_type, count_type)

        return result

    def _get_nodes_by_relation_and_type(self, tx, source_label, source_filter, source_filter_value,
                                            dest_label, dest_filter, dest_filter_value,
                                            relation, return_type, count_type):
